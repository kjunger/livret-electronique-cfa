...
{	
	...
	function connection($login,$mdp){
		...
		{
			$requete = "select id,mdp from utilisateur where login = '$login'";
			$db->requete($requete,1);	
			if($db->nb_resultat(1))
			{
				$resultat = $db->resultat(1);
	...
	{
		global $db;
		$requete = "select id from stage where idStagiaire = ".$this->getId() ." or idResponsableFormation = ".$this->getId();
		$db->requete($requete,1);	
